...
router.get("/modulemenus/:moduleid", (req, res)=>{
    let object = new Module();
    object.id = req.params.moduleid;
    object.getmodulemenus().then((result)=>{
        res.end(JSON.stringify({status:"success", data:result}));
    }, (err)=>{
        res.end(JSON.stringify({status:"failed", data:err}));
    })
...
router.post("/addmodulemenu/:moduleid/:menuid", (req, res)=>{
    let object = new Module();
    object.addmodulemenu(req.params.moduleid, req.params.menuid).then((result)=>{
        res.end(JSON.stringify({status:"success", data:result}));
    }, (err)=>{
        res.end(JSON.stringify({status:"failed", data:err}));
    })
...
router.post("/removemodulemenu/:moduleid/:menuid", (req, res)=>{
    let object = new Module();
    object.removemodulemenu(req.params.moduleid, req.params.menuid).then((result)=>{
        res.end(JSON.stringify({status:"success", data:result}));
    }, (err)=>{
        res.end(JSON.stringify({status:"failed", data:err}));
    })
